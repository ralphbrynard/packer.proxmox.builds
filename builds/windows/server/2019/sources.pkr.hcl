...
source "vsphere-iso" "windows-server-datacenter-dexp" {
  ...
  firmware             = var.vmware_vm_firmware
  CPUs                 = var.vm_cpu_count
  cpu_cores            = var.vm_cpu_cores
  RAM                  = var.vm_mem_size
  RAM_hot_plug         = var.vm_mem_hot_add
  cdrom_type           = var.vm_cdrom_type
  disk_controller_type = var.vmware_disk_controller_type
  storage {
...
source "proxmox-iso" "windows-server-datacenter-dexp" {
  ...
  // Template Settings
  template_name        = var.common_template_conversion ? local.vm_name_datacenter_desktop : null
...
source "proxmox-iso" "windows-server-datacenter-core" {
  ...
  // Template Settings
  template_name        = var.common_template_conversion ? local.vm_name_datacenter_core : null
  template_description = var.common_template_conversion ? local.build_description : null
}
